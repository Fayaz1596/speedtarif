(function (window, document, undefined)
{
    ...
    var media = window.matchMedia('(min-width: 320px) and (max-width: 812px)');
    
    media.addListener(doSomething);
        ...
        if(mediaQuery.matches)
        {
            class Tooltip
            {
                ...
                mouseOver()
                {
                    ...
                    let top = this.element.getBoundingClientRect().top - height + 200 + document.documentElement.scrollTop
                    if(left < 20)
                    {
                        left = 20
                    }
        ...
        else
        {
            class Tooltip
            {
                ...
                mouseOver()
                {
                    ...
                    let width = this.tooltip.offsetWidth
                    let height = this.tooltip.offsetHeight
                    let left = this.element.offsetWidth / 2 - width / 2 + 160 + this.element.getBoundingClientRect().left + document.documentElement.scrollLeft
                    let top = this.element.getBoundingClientRect().top - height + 98 + document.documentElement.scrollTop
                    if(left < 20)
                    {
                        left = 20
                    }
